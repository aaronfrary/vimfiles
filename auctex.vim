...
let b:latex_command = "! pdflatex %"
let b:dvi_viewer_command = "! texworks %:r.pdf"
...
noremap <buffer><silent> <Leader>K :call <SID>NextTexError()<CR>
...
noremap <buffer><silent> <C-K> :call <SID>Xdvi()<CR><Space>
inoremap <buffer><silent> <C-K> <Esc>:call <SID>Xdvi()<CR><Space>
...
function! s:FTwo(var)
	...
	return "\\begin{equation*}\<CR>\<CR>\\end{equation*}\<Up>\<TAB>"
    endif
endfunction
...
function! s:FFour(var)
    if a:var == 0
	return "\\begin{eqnarray*}\<CR>\<CR>\\end{eqnarray*}\<Up>\<TAB>"
    else
	return "\\begin{align*}\<CR>\<CR>\\end{align*}\<Up>\<TAB>"
    endif
endfunction
